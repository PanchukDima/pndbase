...
void Dialog_list_not_work::send_data()
{
    ...
    if(db.open())
    {
        ...
        case 0:
              ...
              query.exec("INSERT INTO test.notwork_list(date_start, date_stop, medcard_id, count_day, staff_add_id, staff_edit_id, delete_row, diagnos_id, who_open, who_close, state) VALUES ('"+date_open_value+"', '"+date_close_value+"', "+global_id+", "+day_count+", "+obj.staff_id+", 0, 'false', "+diagnos_id+", "+who_open+", "+who_close+", '"+state_str+"');");
            ...
            break;
        case 1:
            query.exec("UPDATE test.notwork_list SET date_start='"+date_open_value+"', date_stop='"+date_close_value+"', count_day="+day_count+", staff_edit_id="+obj.staff_id+", diagnos_id="+diagnos_id+", who_open="+who_open+", who_close="+who_close+", state='"+state_str+"' WHERE id = "+global_id+";");
            if(query.lastError().isValid())
            {
                qDebug()<<query.lastError();
                QMessageBox::warning(this,"Ошибка SQL","Произошла ошибка при обращении к базе данных");
